...
def main():
    ...
    parser.add_argument("--config", default="configs/b02_unetpp_kvasir_cvc.yaml")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics["dice"] > best_dice:
            ...
            epochs_without_improvement = 0
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "best_dice": best_dice, "config": config}, ROOT / "checkpoints/unetpp_best.pth")
        else:
            epochs_without_improvement += 1

        with open(ROOT / "logs/unetpp_train_log.csv", "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=list(logs[0].keys()))
            writer.writeheader()
            writer.writerows(logs)
        save_training_curve(pd.DataFrame(logs), ROOT / "results/visualizations/unetpp_training_curve.png")
    ...
    print("Best checkpoint: checkpoints/unetpp_best.pth")
