...
def validate_processed_data(config):
    ...
    if missing:
        print("Processed data is missing. Please run first:")
        print("python scripts/prepare_data.py --seed 42")
        print(f"Missing paths: {missing}")
        sys.exit(1)
    if empty:
        print("Processed data folders are empty. Please run first:")
        print("python scripts/prepare_data.py --seed 42")
...
def main():
    ...
    if augmentation_config.get("enabled", False):
        train_transform = build_train_augmentation(image_size, augmentation_config)
    val_transform = build_eval_transform(image_size)

    train_dataset = PolypDataset(
        ROOT / config["data"]["train_images"],
        ROOT / config["data"]["train_masks"],
        image_size,
        transform=train_transform,
    )
    val_dataset = PolypDataset(
        ROOT / config["data"]["val_images"],
        ROOT / config["data"]["val_masks"],
        image_size,
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            ...
            f"val Dice {val_metrics['dice']:.4f} | val IoU {val_metrics['iou']:.4f} | val MAE {val_metrics['mae']:.4f}"
        )
        ...
        if val_metrics["dice"] > best_dice:
            ...
            torch.save(
                ...
                ROOT / "checkpoints/unet_best.pth",
            )
        else:
            epochs_without_improvement += 1

        with open(ROOT / "logs/unet_train_log.csv", "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=list(logs[0].keys()))
            writer.writeheader()
            writer.writerows(logs)
        save_training_curve(pd.DataFrame(logs), ROOT / "results/visualizations/unet_training_curve.png")
        ...
        if early_stopping_patience and epochs_without_improvement >= early_stopping_patience:
            print(
                ...
                f"Val Dice did not improve for {early_stopping_patience} epochs."
            )
            break

    print(f"Training complete. Best val Dice: {best_dice:.4f}")
    print("Best checkpoint: checkpoints/unet_best.pth")
