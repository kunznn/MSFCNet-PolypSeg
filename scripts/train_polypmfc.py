...
from src.losses.uncertainty_boundary_loss import UncertaintyBoundaryBCEDiceLoss
...
def build_criterion(config):
    ...
    if loss_config.get("use_ubl", False):
        return UncertaintyBoundaryBCEDiceLoss(
            boundary_lambda=loss_config.get("boundary_lambda", 1.0),
...
def main():
    ...
    parser.add_argument("--config", default="configs/a3_polypmfc_msm_bcf_kvasir_cvc.yaml")
    ...
    run_name = config.get("run_name") or slugify(config.get("experiment_name", "polypmfc"))
    checkpoint_path = ROOT / "checkpoints" / f"{run_name}_best.pth"
    log_path = ROOT / "logs" / f"{run_name}_train_log.csv"
    curve_path = ROOT / "results" / "visualizations" / f"{run_name}_training_curve.png"
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics["dice"] > best_dice:
            ...
            epochs_without_improvement = 0
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "best_dice": best_dice, "config": config}, checkpoint_path)
        else:
        ...
        save_training_curve(pd.DataFrame(logs), curve_path)
    ...
    print(f"Best checkpoint: {checkpoint_path.relative_to(ROOT)}")
